...
export default function ContactDetailPage() {
  ...
  }, [contact, contacts, router]);

  if (!contact) {
    return (
      <div className="flex-1 flex flex-col items-center justify-center p-12 text-slate-500">
        <p className="text-sm font-medium">Loading profile or contact not found...</p>
        <Link href="/contacts" className="mt-4 text-xs font-semibold text-purple-600 hover:underline">
          Return to contacts
        </Link>
      </div>
    );
  ...
  return (
    <div className="flex-1 pb-10">
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex items-center gap-2 self-end">
            <button
              ...
              className="flex items-center gap-1 px-3 py-1.5 text-xs font-semibold text-slate-600 bg-white border border-slate-200 hover:bg-slate-50 rounded-lg shadow-sm transition"
            >
            ...
            <button
              ...
              className="flex items-center gap-1 px-3 py-1.5 text-xs font-semibold text-red-600 bg-white border border-red-200 hover:bg-red-50 rounded-lg shadow-sm transition"
            >
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <div className="flex items-center gap-2.5">
                ...
                <span className={`px-2 py-0.5 text-[10px] font-semibold tracking-wide uppercase rounded-full border ${statusColors[contact.status]}`}>
                  {contact.status}
                </span>
              </div>
          ...
          <div className="md:col-span-1 space-y-6">
            ...
            <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm space-y-3">
              ...
              <p className="text-xs text-slate-600 leading-relaxed italic">
                {contact.notes || "No context notes added yet. Edit profile to write notes."}
              </p>
            </div>

          </div>

          {/* Right Columns: Timeline and reminders */}
          <div className="md:col-span-2 space-y-6">
            
            {/* Top checklist followups */}
            <div className="bg-white border border-slate-200 rounded-xl p-6 shadow-sm">
              <div className="flex items-center justify-between mb-4 border-b border-slate-100 pb-3">
                ...
                <button
                  ...
                  className="flex items-center gap-1 px-2.5 py-1.5 text-xs font-semibold text-purple-700 bg-purple-50 hover:bg-purple-100 border border-purple-200 rounded-lg transition"
                >
              ...
              {contactReminders.length === 0 ? (
                <div className="py-8 text-center text-xs text-slate-400 bg-slate-50/50 border border-dashed border-slate-200 rounded-lg">
                      ...
                      <div key={rem.id} className="flex items-center justify-between py-3 first:pt-0 last:pb-0">
                        <div className="flex items-start gap-3 min-w-0">
                          ...
                          <div className="min-w-0">
                            <span className={`text-xs font-medium ${rem.completed ? 'line-through text-slate-400' : 'text-slate-900'}`}>
                              {rem.title}
                            </span>
                        ...
                        <button
                          ...
                          className="text-slate-400 hover:text-red-600 transition shrink-0 ml-4"
                        >
                          <Trash2 className="w-3.5 h-3.5" />
                        </button>
                      </div>
            ...
            <div className="bg-white border border-slate-200 rounded-xl p-6 shadow-sm">
              <div className="flex items-center justify-between mb-6 border-b border-slate-100 pb-3">
                ...
                <button
                  ...
                  className="flex items-center gap-1 px-2.5 py-1.5 text-xs font-semibold text-slate-600 bg-white border border-slate-200 hover:bg-slate-50 rounded-lg transition"
                >
                  <Plus className="w-3.5 h-3.5" />
              ...
              {contactInteractions.length === 0 ? (
                <div className="py-12 text-center text-xs text-slate-400 bg-slate-50/50 border border-dashed border-slate-200 rounded-lg">
                        ...
                        <div className="space-y-1">
                          <div className="flex items-center justify-between gap-4">
                            <div className="flex items-center gap-2">
                              <span className={`text-[10px] font-semibold tracking-wide uppercase px-1.5 py-0.5 rounded border ${typeColors[int.type]}`}>
                                {int.type}
                              </span>
                            ...
                            <button
                              ...
                              className="text-slate-400 hover:text-red-600 opacity-0 group-hover:opacity-100 transition shrink-0 p-1"
                            >
                              <Trash2 className="w-3.5 h-3.5" />
                            </button>
                          </div>
                          ...
                          {int.notes && (
                            <p className="text-xs text-slate-500 leading-relaxed bg-slate-50 p-2.5 rounded-lg border border-slate-100 mt-1.5">
                              {int.notes}
                            </p>
                          )}
                        </div>
      ...
      <ConfirmationDialog 
        ...
        description="Are you sure you want to delete this contact profile? This will delete all of their history, notes, logged interactions, and reminders. This action cannot be undone."
